...
```{r}
chosen_hvgs <- getTopHVGs(gene_var, prop=0.025)
```

```{r}
scae <- scae[unique(c(chosen_hvgs, immune_features)), ]
```
...
create_dim_red <- function(scae){
  genes <- c(rownames(scae_subset(scae, "nonimmune")), rownames(scae_subset(scae, "immune_genes")))
  gene_var <- modelGeneVar(scae[genes, ])
  chosen_hvgs <- getTopHVGs(gene_var, prop=0.02)
  immune_features <- grep("^ENS", rownames(scae), invert=TRUE, value=TRUE)
  scae <- scae[unique(c(chosen_hvgs, immune_features)), ] # do NOT do this - this is executed here for memory reasons
  scae <- runPCA(scae, subset_row=chosen_hvgs, name="PCA_g")
