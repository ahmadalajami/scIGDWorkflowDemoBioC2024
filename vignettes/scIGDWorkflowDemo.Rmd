...
For simplicity, we here perform dimensionality reduction and clustering on gene level, i.e., using immune and non-immune genes, after choosing top 2.5% HVG (for memory/demonstration reasons):
...
create_dim_red <- function(scae){
  ...
  chosen_hvgs <- getTopHVGs(gene_var, prop=0.025) # you could use 20%
  immune_features <- grep("^ENS", rownames(scae), invert=TRUE, value=TRUE)
  scae <- scae[unique(c(chosen_hvgs, immune_features)), ] # do NOT do this - this is executed here for memory/demonstration reasons
